...
const NotificationBell = () => {
  ...
  const handleOpen = async () => {
    const next = !open;
    setOpen(next);
    if (next) await loadNotificari();
  };

  const handleMarkCitit = async (id) => {
    try {
      await axios.put(`${API}/api/notifications/${id}/citit`, {}, { headers: getHeaders() });
      setNotificari(prev => prev.map(n => n._id === id ? { ...n, citit: true } : n));
  ...
  const handleMarkToateCitite = async () => {
    try {
      setLoading(true);
      await axios.put(`${API}/api/notifications/citit-toate`, {}, { headers: getHeaders() });
  ...
  const handleDelete = async (id, e) => {
    e.stopPropagation();
    try {
      await axios.delete(`${API}/api/notifications/${id}`, { headers: getHeaders() });
      setNotificari(prev => prev.filter(n => n._id !== id));
      const deleted = notificari.find(n => n._id === id);
      if (deleted && !deleted.citit) setTotalNecitite(prev => Math.max(0, prev - 1));
    } catch (err) {}
  };
  ...
  const getTipColor = (tip) => {
    const colors = { reminder: '#6366f1', milestone: '#f4d03f', intarziere: '#ef4444', sistem: '#6b7280' };
  ...
  const panel = open ? ReactDOM.createPortal(
    <div
      ...
      style={{
        ...
        padding: isMobile ? '0' : '70px 20px 0 0'
      }}
    >
      <div
        ref={panelRef}
        ...
        style={{
          ...
          animation: isMobile ? 'notifSheetUp 0.25s ease' : 'notifFadeIn 0.2s ease'
        }}
      >
        {/* Header */}
        <div style={{
  ...
  return (
    <>
      <button
        ...
        style={{
          position: 'relative',
          background: open ? 'rgba(99,102,241,0.1)' : 'var(--bg-card)',
